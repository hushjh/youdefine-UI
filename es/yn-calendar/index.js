...
var push$1 = function push(arr, ele) {
  if (Array.isArray(arr)) {
    if (Array.isArray(ele)) {
      ele.forEach(function (item) {
...
var YnDate = function YnDate() {
  for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
    args[_key] = arguments[_key];
  }

  return _construct(YnDate.fn.Init, args);
};
...
YnDate.prototype = YnDate.fn = {
  ...
  add: function add() {
    ...
    if (["days", "day", "d"].indexOf(unit) > -1) {
      ...
      var otherMonthDaysCount = YnDate(this.year, this.JSDate.getMonth() + 1 + count, "01").getDaysCountOfMonth();
  ...
  diff: function diff() {
    ...
    if (args.length >= 2) {
      ...
      // 两个时间差，默认是传入时间和当前时间差
      return (YnDate(year, month, date).getTime() - this.getTime()) / 1000 / 60 / 60 / 24;
    } else {
      if (year instanceof YnDate) {
...
var YnCalendar = defineComponent({
  name: "Calendar",
  ...
  methods: {
    ...
    haneleSingleMode: function haneleSingleMode(date) {
      this.changedNode = {};
      push$1(date.className, ["active", "single-mode"]);
    ...
    haneleDoubleMode: function haneleDoubleMode(date) {
      if (!this.fromDate) {
        ...
        push$1(date.className, ["start", "active"]);
        ...
        if (!this.toDate) {
          if (date.key === this.fromDate.key) {
            if (this.doubleModeAllowSameDate) {
              ...
              push$1(this.changedNode[this.fromDate.key].className, "end");
            ...
            if (this.fromDate.ynDate.isAfter(date.ynDate)) {
              ...
              this.confirmButtonClassName = "active";
              push$1(date.className, "end");
              this.setDateValue(date, "mark", this.toDateMark);
              this.toDate = date;
            }

            push$1(date.className, "active");
            this.changedNode = _objectSpread2(_objectSpread2({}, this.changedNode), {}, _defineProperty({}, date.key, date));
    ...
    getTimePeriod: function getTimePeriod() {
      var beginDate = YnDate().substract(Number(this.before), this.unit);
      var endDate = YnDate().add(Number(this.after), this.unit);
    ...
    generateDate: function generateDate() {
      ...
      caculatedMonth.forEach(function (item) {
        ...
        var daysOfMonth = YnDate(year, month).getDaysCountOfMonth();

        for (var i = 1; i <= daysOfMonth; i++) {
          var j = i < 10 ? "0".concat(i) : String(i);

          if (j === "01") {
            for (var k = 0; k < YnDate(year, month, j).getDay(); k++) {
          ...
          var className = [];
          var ynDate = YnDate(year, month, j);
          push$1(className, YnDate().isAfter(year, month, j) ? "disable" : "clickable");

          if (_this.mode === "double" && _this.fromDate && _this.toDate) {
            push$1(className, YnDate(year, month, j).isBetween(_this.fromDate.ynDate, _this.toDate.ynDate) ? "during-active" : "");
          }

          if (_this.beginDate && _this.endDate) {
            if (YnDate(year, month, j).isBetweenIncludeBoth(_this.beginDate, _this.endDate)) {
              push$1(className, ["open-days", "clickable"]);
              drop(className, "disable");
            } else {
              push$1(className, "disable");
          ...
          var festival = YnDate().isSame(year, month, j) ? _this.todayMark : "";
    ...
    getDefaultNodeFromProps: function getDefaultNodeFromProps(prop) {
      ...
      var festival = YnDate().isSame(year, month, day) ? this.todayMark : "";
      var ynDate = YnDate(year, month, day);
      return {
        key: key,
        ynDate: ynDate,
        year: year,
      ...
      dates.forEach(function (monthItem) {
        caculateDOM.push(h("div", {
          ...
          monthItem.dates.forEach(function (date) {
            ...
            dateDom.push(h(genComponentName("flex-item"), {
              key: date.key,
            ...
            }, date.festival), h(genComponentName("flex-item"), {
              class: ["yn-calendar-date-text"]
            }, date.day), h(genComponentName("flex-item"), {
    ...
    createCloseIcon: function createCloseIcon(h) {
      return h(genComponentName("iconfont"), {
    ...
    createWeekBar: function createWeekBar(h) {
      ...
      for (var i = 0; i < 7; i++) {
        bar.push(h(genComponentName("flex-item"), {
          key: i
        }, [h("span", {
          domProps: {
  ...
  render: function render(h) {
    return h("div", {
      ...
      ref: "header"
    }, [this.createHeaderArea(h)]), h(genComponentName("flex-item"), {
      ref: "scroller",
...
var index$1 = install(YnCalendar);
